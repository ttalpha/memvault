...
export async function fetchUserFromDB(userId: string) {
  console.log(`[APP] Simulating DB fetch for user ${userId}...`);
  return new Promise((resolve) =>
    setTimeout(() => {
      const userData = mockUsers.find((user) => user.id === userId);
      console.log(`[APP] Fetched user ${userId} from DB.`);
      resolve(userData);
    }, 1000)
  );
}
...
export async function updateUser(
  ...
  console.log(`[APP] Simulating DB update for user ${userId}...`);
...
export async function deleteUser(
  ...
  console.log(`[APP] Simulating DB delete for user ${userId}...`);
  return new Promise((resolve) =>
    setTimeout(() => {
      const index = mockUsers.findIndex((user) => user.id === userId);
